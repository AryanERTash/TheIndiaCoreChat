...
# FastAPI setup
# ─────────────────────────────────────────────
app = FastAPI(title="TheIndiaCore Chatbot", version="1.0")
...
# ─────────────────────────────────────────────
# Chat Endpoint — WITH CONVERSATION HISTORY (POST)
# ─────────────────────────────────────────────
...
async def chat(request: Request, chat_request: ChatRequest):
    try:
        ...
        system_prompt = chat_request.system_prompt if chat_request.system_prompt else SYSTEM_PROMPT
        ...
        result = await run_with_timeout(
            chain.ainvoke({"user_input": chat_request.message}),
            timeout_sec=25
        )
...
@app.get("/health")
async def health():
    return {"status": "ok", "service": "TheIndiaCore Chatbot"}
...
async def startup_event():
    print("\n" + "=" * 60)
    print("🚀 TheIndiaCore Chatbot Server Started!")
    print("=" * 60)
    print(f"📍 Server URL: http://localhost:8000")
    print(f"🌐 Chatbot UI: http://localhost:8000/")
